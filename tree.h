...
// 
// 特点：
//	树是一种非线性存储结构，通常用来表示逻辑关系为“一对多”的数据。
// 
// 存储：
//	1. 顺序存储：双亲表示法；
//	2. 链式存储：孩子表示法，孩子兄弟表示法。
// 
// 双亲表示法：
//	使用数组存储树的所有结点。
//	数组结点包含两个域：数据域存放树结点信息，双亲域存放树结点的双亲结点在数组中的位置。
//	特点：找双亲易，找孩子难。
// 
// 孩子表示法（顺序表 + 链表）：
//	使用数组存储树的所有结点，同时为每个树结点配备一个链表，记录该树结点的所有孩子结点。
//	数组结点包含两个域：数据域存放树结点信息，孩子域存放树结点的子结点链表的头指针；
//	链表结点包含两个域：数据域存放子结点在数组中的位置，指针域指向下个链表结点。
//	特点：
//		1. 找孩子易，找双亲难；
//		2. 可为链表结点增加双亲域，存放子结点的双亲结点在数组中的位置，达到：找孩子易，找双亲易。
// 
// 孩子兄弟表示法（二叉链表）：
//	使用二叉链表（定义见下方二叉树部分）存储树的所有结点。
//	链表结点包含三个域：数据域存放树结点信息，左指针域指向树结点第一个（从左到右）孩子；右指针域指向树结点右方第一个兄弟。
//	特点：
//		1. 找孩子易，找双亲易；
//		2. 可以将普通树转化为二叉树。
// 
...
typedef struct
{
	PTNode nodes[TREE_MAX_SIZE]; // 存储树所有结点的数组
	int r; // 根结点在数组中的位置
	int n; // 树包含的结点个数
}PTree;
...
{
	int position; // 子结点在数组中的位置
	struct CTNode* p_next; // 指向下个链表结点的指针
}CTNode;
...
typedef struct
{
	CTBox nodes[TREE_MAX_SIZE]; // 存储树所有结点的数组
	int r; // 根结点在数组中的位置
	int n; // 树包含的结点个数
}CTree;
...
{
	ElemType e; // 树结点信息
	struct CSNode* p_child; // 指向树结点第一个（从左到右）孩子
	struct CSNode* p_sibling; // 指向树结点右方第一个兄弟
}CSNode, *CSTree;
...
// 
// 顺序存储：
//	使用数组存储，适用于存储完全二叉树（包括满二叉树），将树结点从上到下、从左到右依次存入数组中；
//	若要存储一般二叉树，需要先将二叉树虚拟填充为完全二叉树，虚拟添加的结点对应在数组中存一个无效值，会造成空间浪费；
//	若要存储一般二叉树，最坏的情况下，即深度为 k 且只有 k 个结点的右单支树，需要使用的数组长度为 [(2 的 k 次方)-1]。
// 
// 链式存储：
//	使用链表存储，可以存储各种类型的二叉树；
...
typedef struct BiTNode
{
	...
	// struct BiTNode* p_par; // 指向双亲的指针，若包含该指针，则结点组成的链表为三叉链表
